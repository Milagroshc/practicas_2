...
class PersonaControlador{
	...
	public function index(Request $request, Response $response, $args){
		...
		//		exit;
				...
				$datin["parametros"] = ['title'=>"Registro de Persona :: negocio", 'titulo' => "Gestión de personas" ];
				//var_dump($datin["estados"]["data"]["tabtablas"]);
				//exit;
				$this->view->render($response, 'negocio/pedidos/usuarios/persona.twig', $datin);
				return $response;
	}

	// RETORNA DATOS DEL USUARIO EN CASO EXISTA
	public function getColumnasPersona(Request $request, Response $response, $args){
		//$columnas =Persona::getTableColumns();
		$filas = Persona::getTableFilas();
		$columnas = array_keys($filas["data"][0]);
		$caracteres = array("_");
	...
	public function getFilasPersona(Request $request, Response $response, $args){
		$filas =Persona::getTableFilas();
		return JsonRenderer::RespuestaJSON($response,$filas["data"]);
	}

	//valioda persona por ajax para registro de persona y creacion de usuario
	public function validaPersonaAjax(Request $request, Response $response, $args){
		try {
			$datos = $request->getParsedBody(); /*se utiliza para recibir parametros post*/

			$datin["persona"]=Persona::getPersonaByTipDocNro($datos);
	...
	/*crear usuario*/
	public function validaPersona(Request $request, Response $response, $args){
		$datos = $request->getParsedBody();
		$rpta = Persona::validaPersona($datos);
		return JsonRenderer::RespuestaJSON($response,$rpta);
	}
	...
	public function DatosUsuarioByUsername(Request $request, Response $response, $args){
		...
		return PersonaControlador::RespuestaJSON($response,$data);
	}
...
// ******************************************************************************* MANTENIMIENTO DE USUARIOS *****************************************************************************
	...
	public function NuevoUsuario(Request $request, Response $response, $args) {
		...
		$datin["mensaje"] = $mensaje;
		$datin["success"] = $estado;
		return PersonaControlador::RespuestaJSON($response,$datin);
	}
